...
* [`Variable`](kicad-addon/library/footprints/Kikakuka.pretty/Variable.kicad_mod) displays its `Value` on the board. Use build-variant fields such as `Value#Flag` or `Value#Option=Choice` to show a value selected by the active build flags or options.
* [`StringTemplate`](kicad-addon/library/footprints/Kikakuka.pretty/StringTemplate.kicad_mod) formats its `Value` during export. Braced placeholders are replaced with matching footprint properties or build options after build-variant fields have been applied.
* [`CouplerFixed`](kicad-addon/library/footprints/Kikakuka.pretty/CouplerFixed.kicad_mod) and [`CouplerMoving`](kicad-addon/library/footprints/Kikakuka.pretty/CouplerMoving.kicad_mod) define matching planes for [coupler-based PCB alignment](#freecad-integration). Give a pair the same reference and use their `Z`, `Offset`, and `Tilt` properties as needed. [`CouplerAt`](kicad-addon/library/footprints/Kikakuka.pretty/CouplerAt.kicad_mod) instead aligns its PCB to absolute FreeCAD world `TargetX`, `TargetY`, and `TargetZ` coordinates, all defaulting to `0 mm`. Place it on B.Cu for the usual behavior, where the PCB bottom surface is positioned at `TargetZ`; use F.Cu only to reference the top surface. All three coupler footprints use the `Unspecified` type, placing their helper models under **Virtual Models** in KiCad's 3D Viewer.
