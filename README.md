...
* FreeCAD
    ...
    * [Coupler-Based PCB Alignment](#coupler-based-pcb-alignment)
        ...
        * FreekiCAD checks the saved coupler list against KiCad's live document every second, including unsaved position, side, rotation, `Z`, and `Tilt` edits. A changed pose updates its marker and recalculates linked-board positioning. The coupler child object's `X`, `Y`, `Z`, and `Tilt` properties are editable in FreeCAD; edits update the marker and linked-board positioning immediately and are written back to the live KiCad footprint. `Y` is shown using FreeCAD's axis convention. Adding, removing, or changing the identity of couplers takes effect on the next PCB reload, which rebuilds the saved list.
        * The [`kicad-addon`](kicad-addon) directory provides [`CouplerFixed`](kicad-addon/footprints/kikakuka.pretty/CouplerFixed.kicad_mod), [`CouplerMoving`](kicad-addon/footprints/kikakuka.pretty/CouplerMoving.kicad_mod), and [`CouplerOrigin`](kicad-addon/footprints/kikakuka.pretty/CouplerOrigin.kicad_mod).
...
* [`StringTemplate`](kicad-addon/footprints/kikakuka.pretty/StringTemplate.kicad_mod) formats its `Value` during export. Braced placeholders such as `{Revision}` are replaced with matching footprint properties or build options after build-variant fields have been applied.
* [`CouplerFixed`](kicad-addon/footprints/kikakuka.pretty/CouplerFixed.kicad_mod) and [`CouplerMoving`](kicad-addon/footprints/kikakuka.pretty/CouplerMoving.kicad_mod) define matching planes for [coupler-based PCB alignment](#freecad-integration). Give a pair the same reference. [`CouplerOrigin`](kicad-addon/footprints/kikakuka.pretty/CouplerOrigin.kicad_mod) instead aligns its PCB with the world origin. Use the `Z` and `Tilt` properties when the plane is offset or tilted from the PCB surface.
