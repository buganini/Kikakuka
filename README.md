...
* FreeCAD Integration (only tested on macOS/Windows)
    ...
    * Linked objects work with the `Assembly` and `Manipulator` workbenches, as well as FreeCAD's built-in transform tool; FreeCAD Assembly can place `PcbObject` Links but currently cannot resolve their child component or connector faces for joints; exporting selected `App::Link` instances or an Assembly to `.kkkk_asm` flattens their final global placements
...
* FreeCAD
    ...
    * [Coupler-Based PCB Alignment](#coupler-based-pcb-alignment)
        ...
        * After any linked PCB reloads, positioning is recalculated for every linked PCB in dependency order. A concurrently reloading PCB is skipped until its fresh poses and bent markers are available; its completion triggers another full pass. A PCB with `SnapToCoupler` enabled (the default) is moved as a whole so that its moving coupler plane meets the matching fixed coupler plane face-to-face; disabling it excludes that PCB from automatic positioning. Alignment uses each plane's position after flex-PCB bending. Each coupler is also available as a visible child object in FreeCAD so its plane can be inspected directly.
        * FreekiCAD checks the saved coupler list against KiCad's live document every second, including unsaved position, side, rotation, local-plane, and `CouplerAt` target edits. A changed pose updates its marker and recalculates linked-board positioning. The coupler child object's footprint position and local-plane properties are editable in FreeCAD; edits update the marker and linked-board positioning immediately and are written back to the live KiCad footprint. `Y` is shown using FreeCAD's axis convention. Adding, removing, or changing the identity of couplers takes effect on the next PCB reload, which rebuilds the saved list.
        * The [`kicad-addon`](kicad-addon) directory provides [`CouplerFixed`](kicad-addon/footprints/Kikakuka.pretty/CouplerFixed.kicad_mod), [`CouplerMoving`](kicad-addon/footprints/Kikakuka.pretty/CouplerMoving.kicad_mod), and [`CouplerAt`](kicad-addon/footprints/Kikakuka.pretty/CouplerAt.kicad_mod).
        ...
        * The coupler plane is defined by the footprint position, side, rotation, and these custom footprint properties:
            * `CouplerFixed` and `CouplerMoving` use `Z` to move the plane origin along the footprint's local Z axis. It defaults to `0 mm`; values without a unit are millimetres, and `mm`, `in`, `mil` (`0.001 in`), and `um` are supported. The origin starts at the PCB surface, including the board thickness on F.Cu. The local Z direction is reversed on B.Cu.
...
Coupler plane markers are available as visible child objects in FreeCAD.
...
# Kikakuka Footprints
The footprint library is stored under [`kicad-addon`](kicad-addon). Add [`kicad-addon/footprints/Kikakuka.pretty`](kicad-addon/footprints/Kikakuka.pretty) to KiCad's footprint library table (for example, as `Kikakuka`).

* [`Variable`](kicad-addon/footprints/Kikakuka.pretty/Variable.kicad_mod) displays its `Value` on the board. Use build-variant fields such as `Value#Flag` or `Value#Option=Choice` to show a value selected by the active build flags or options.
* [`StringTemplate`](kicad-addon/footprints/Kikakuka.pretty/StringTemplate.kicad_mod) formats its `Value` during export. Braced placeholders are replaced with matching footprint properties or build options after build-variant fields have been applied.
* [`CouplerFixed`](kicad-addon/footprints/Kikakuka.pretty/CouplerFixed.kicad_mod) and [`CouplerMoving`](kicad-addon/footprints/Kikakuka.pretty/CouplerMoving.kicad_mod) define matching planes for [coupler-based PCB alignment](#freecad-integration). Give a pair the same reference and use their local `Z` and `Tilt` properties as needed. [`CouplerAt`](kicad-addon/footprints/Kikakuka.pretty/CouplerAt.kicad_mod) instead aligns its PCB to absolute FreeCAD world `TargetX`, `TargetY`, and `TargetZ` coordinates, all defaulting to `0 mm`. Place it on B.Cu for the usual behavior, where the PCB bottom surface is positioned at `TargetZ`; use F.Cu only to reference the top surface.
