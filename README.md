...
* [`CouplerFixed`](kicad-addon/footprints/Kikakuka.pretty/CouplerFixed.kicad_mod) and [`CouplerMoving`](kicad-addon/footprints/Kikakuka.pretty/CouplerMoving.kicad_mod) define matching planes for [coupler-based PCB alignment](#freecad-integration). Give a pair the same reference and use their `Z`, `Offset`, and `Tilt` properties as needed. [`CouplerAt`](kicad-addon/footprints/Kikakuka.pretty/CouplerAt.kicad_mod) instead aligns its PCB to absolute FreeCAD world `TargetX`, `TargetY`, and `TargetZ` coordinates, all defaulting to `0 mm`. Place it on B.Cu for the usual behavior, where the PCB bottom surface is positioned at `TargetZ`; use F.Cu only to reference the top surface.
