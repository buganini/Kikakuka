...
* FreeCAD
    ...
    * [Coupler-Based PCB Alignment](#coupler-based-pcb-alignment)
        ...
        * The [`kicad-library`](kicad-library) directory provides [`CouplerFixed`](kicad-library/footprints/Kikakuka.pretty/CouplerFixed.kicad_mod), [`CouplerMoving`](kicad-library/footprints/Kikakuka.pretty/CouplerMoving.kicad_mod), and [`CouplerAt`](kicad-library/footprints/Kikakuka.pretty/CouplerAt.kicad_mod).
...
# Kikakuka Library
The KiCad library is stored under [`kicad-library`](kicad-library). Add [`kicad-library/footprints/Kikakuka.pretty`](kicad-library/footprints/Kikakuka.pretty) to KiCad's footprint library table (for example, as `Kikakuka`). Its [`Kikakuka.3dshapes`](kicad-library/3dmodels/Kikakuka.3dshapes) directory contains the unit-cube placeholder and optional coupler helper models.

* [`Variable`](kicad-library/footprints/Kikakuka.pretty/Variable.kicad_mod) displays its `Value` on the board. Use build-variant fields such as `Value#Flag` or `Value#Option=Choice` to show a value selected by the active build flags or options.
* [`StringTemplate`](kicad-library/footprints/Kikakuka.pretty/StringTemplate.kicad_mod) formats its `Value` during export. Braced placeholders are replaced with matching footprint properties or build options after build-variant fields have been applied.
* [`CouplerFixed`](kicad-library/footprints/Kikakuka.pretty/CouplerFixed.kicad_mod) and [`CouplerMoving`](kicad-library/footprints/Kikakuka.pretty/CouplerMoving.kicad_mod) define matching planes for [coupler-based PCB alignment](#freecad-integration). Give a pair the same reference and use their `Z`, `Offset`, and `Tilt` properties as needed. [`CouplerAt`](kicad-library/footprints/Kikakuka.pretty/CouplerAt.kicad_mod) instead aligns its PCB to absolute FreeCAD world `TargetX`, `TargetY`, and `TargetZ` coordinates, all defaulting to `0 mm`. Place it on B.Cu for the usual behavior, where the PCB bottom surface is positioned at `TargetZ`; use F.Cu only to reference the top surface. All three coupler footprints use the `Unspecified` type, placing their helper models under **Virtual Models** in KiCad's 3D Viewer.

The separate Kikakuka Tools IPC plugin provides **Coupler 3D Viewer** and **Hide Couplers** actions. Coupler 3D Viewer applies each coupler's `Z`, `Tilt`, and `Offset` to its helper model and opens KiCad's 3D Viewer; Hide Couplers marks these models hidden without removing them and also opens the Viewer. **Add Placeholder 3D Models** likewise opens the Viewer after updating models. To omit couplers from STEP export, enable **Ignore 'Unspecified' components**.
